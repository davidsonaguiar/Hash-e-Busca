...
Assim como no teste de busca criei um gerador de dados, mas nesse caso, simulei CPFs aleatórios que não se repetem, para isso utilizei set.
...
E também criei uma funcão para cada estrutura, essa funcões recebem os dados gerados e executão a inserção do registro na estrura para cada registro dos dados recebidos.
...
Por fim, criei a função `teste_desempenho_inserir`, para executar os testes para cada estrutra e tamanhos.
...
```
  ...
  **Resultado**
  ...
  - Inserção Sequêncial:
    - Teste com 10.000 registros: 1.350400s
  ...
  - Inserção Binária:
    ...
    - Teste com 10.000 registros: 0.018922s

    - Teste com 100.000 registros: 0.182175s

    - Teste com 1.000.000 registros: 2.318447s

  - Inserção Hash:

    - Teste com 10.000 registros: 0.217678s

    - Teste com 100.000 registros: 4.939084s

    - Teste com 1.000.000 registros: 297.232782s


## Análise dos Resultados dos Testes de Desempenho de Inserção e Busca:

**Inserção:**

- **Sequencial:** Os tempos de inserção crescem significativamente com o aumento do número de registros, o que indica uma complexidade de tempo linear (O(n)). Isso ocorre porque a inserção sequencial requer deslocamento de elementos para abrir espaço para novos registros.

- **Binária:** A inserção binária mostra um desempenho melhor em comparação com a sequencial, com tempos de inserção consideravelmente menores. No entanto, também apresenta um aumento no tempo de inserção com o aumento do número de registros, devido ao custo da busca binária para encontrar a posição correta de inserção.

- **Hash:** A inserção com tabela hash tem tempos relativamente baixos para 10.000 registros, mas cresce rapidamente com o aumento do número de registros. Isso pode ocorre devido à necessidade de re-hash e resolução de colisões, especialmente quando a tabela está quase cheia. O tempo de inserção se torna muito alto para 1.000.000 de registros, indicando uma complexidade de tempo próxima de O(n).

**Busca:**

- **Sequencial:** O tempo de busca aumenta linearmente com o número de registros, refletindo a complexidade de tempo O(n). Isso ocorre porque a busca sequencial percorre todos os registros até encontrar o desejado.

- **Binária:** A busca binária apresenta tempos de busca muito baixos e consistentes, independentemente do número de registros. Isso é característico da complexidade de tempo logarítmico (O(log n)).

- **Hash:** A busca com tabela hash apresenta tempos extremamente baixos e consistentes, independentemente do número de registros.
